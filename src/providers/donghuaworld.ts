...
const SITE_CONFIG = {
    id: 'donghuaworld',
    name: 'Donghuaworld',
    mainUrl: 'https://donghuaworld.com',
    lang: 'zh',
    serverSelector: 'div.server-item a',
    serverAttr: 'data-hash',
};
...
const donghuaworldProvider: Provider = {
    id: SITE_CONFIG.id,
    ...
    async getCatalog(type: string, extra: any): Promise<Meta[]> {
        ...
        try {
            ...
            console.error(`[Donghuaworld] Catalog error:`, err);
            return [];
        }
    },
        ...
        try {
            ...
            if (item.id.startsWith(`agg:${SITE_CONFIG.id}:`)) {
                ...
                for (const query of searchQueries) {
                    ...
                    if (candidates.length > 0) {
                        ...
                        console.log(`[Donghuaworld] Matched search result: "${candidates[0].title}"`);
                        break;
                    }
                }
            }
            ...
            $w(SITE_CONFIG.serverSelector).each((_, el) => {
                ...
                try {
                    ...
                    console.error(`[Donghuaworld] Decode error:`, e);
                }
            });
            ...
            console.error(`[Donghuaworld] Error:`, message);
            return [];
        }
    }
};

export default donghuaworldProvider;
